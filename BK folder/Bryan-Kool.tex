...
points}).
\item In section~\ref{sec: pushforward to sym prod}, we pushforward
the Euler characteristic measure to $\Sym B$ via the map $\rho$. We
show that $\rho_{*}(1)$, the pushforward measure, has nice
...
products}). 
\item To compute the pushforward measure $\rho_{*}(1)$ explicitely, we
...
comb curve $C$. The moduli of this stratum comes solely from the ways
...
using a fpqc cover of $X$ which includes the formal neighborhoods of
...
to formal nghds}. 
\item After further pushforwards to further symmetric products, we
...
\item Finally, using the trace formulas for the topologocal vertex
...
We write $F_x$ for the fibre $p^{-1}(x)$ over a  point $x \in
...
\section{The reduction to partition thickened comb curves}\label{sec: reduction to thickened comb curves}
...
is a 0-dimensional subscheme which is set-theoretically supported on
$B$.
\end{definition}
...
\begin{equation}\label{eqn: monomial ideal I given by a partion lambda}
I_{\lambda}=(r^{\lambda_{1}},r^{\lambda_{2}}s,\dotsc
,r^{\lambda_{l}}s^{l-1},r^{l}).
\end{equation}\marginpar{Convention check!}
...
\end{theorem}
 ...
 \marginpar{Insert Martijn's proof but adapted to the theorem as
...
where
\[
G(\bolda \boldx ) = \prod_{i=1}^{n}g(a_{i}), \quad H(\bolda \boldx ) =
\prod_{j=1}^{n}h(b_{j}).
\]
\end{proposition}
...
\section{Restriction to formal neighbourhoods} \label{sec: restriction
to formal nghds}
...
\]
are open in the fpqc topology \cite[find exact reference]{???} so
$\mathfrak{U}$ forms an fpqc cover. For $U\in \mathfrak{U}$, we
suppress the associated map $U\to X$ from the notation and use the
usual restriction notation to denote the pullback, for example:
\[
U|_{C}:=U\times_{X}C. 
\]
...
\begin{align*}
\chi (\O_{Z})&=\chi (\O_{C}) +\operatorname{length}(I_{C}/I_{Z})\\
&=\chi (\O_{C}) +\sum_{U\in \mathfrak{U}}\operatorname{length}(I_{C}/I_{Z}|_{U})\\
\end{align*}
therefore the $p^{\chi (\O_{Z})}$ term on the left hand side of the
equation correctly matches the 
\[
p^{\chi (\O_{C})}\cdot \prod_{U\in
\mathfrak{U}}p^{\operatorname{length}(I_{C}/I_{Z}|_{U})}
\]
term on
the right hand side.
\end{proof}
...
(c.f. equation~\eqref{eqn: monomial ideal I given by a partion
...
\]
where \marginpar{Convention check!} 
\begin{align*}
...
\sfVtilde^{e(F^{\circ}_{x_{i}})}_{\lambda^{(i)}\emptyset \emptyset} \cdot  \prod_{j=1}^{m} \sfVtilde_{\bx \mu^{(j)}\emptyset} \cdot \sfVtilde_{\mujprime \mu^{(j)}\emptyset }\cdot 
...
p^{-\mu_{1}^{(j)}}  \sfVtilde_{\bx \mu^{(j)}\emptyset} \cdot \sfVtilde_{\mujprime \mu^{(j)}\emptyset }\cdot 
\sfVtilde^{e(F^{\circ}_{y_{j}})}_{\mu^{(j)}\emptyset \emptyset} 
\end{align*}
...
&\quad \cdot \prod_{j=1}^{m} \sum_{\mu^{(j) \vdash b_{j}}}
p^{\mu_{1}^{(j)}} \cdot \frac{\sfVtilde_{\mujprime 
\mu^{(j)} 
...
\right). 
\]
We can compute the Euler characteristics using the $T=(\CC^{*})^{3}$
action on the Hilbert schemes induced by the $T$ action on
...
corresponds to a 3D parition $\pi \in \left(\ZZ_{\geq 0} \right)^{3}$
by
\[
...
The following hold:  \marginpar{Convention check!}
\[
\sfV_{\lambda \emptyset \emptyset} =\sfVtilde_{\lambda \emptyset
\emptyset}, \quad \sfV_{\bx \lambda \emptyset}  = p^{-\lambda_{1}}
\sfVtilde_{\bx \lambda \emptyset}, \quad \sfV_{\mu '\mu \emptyset}
=p^{-||\mu ||^{2}} \sfVtilde_{\mu '\mu \emptyset} 
\]
where $||\mu ||^{2}:= \sum_{j=1}^{\infty} \mu_{j}^{2}$ .
\end{lemma}
\begin{proof}
...
\]
For $\pi_{min}$, the minimal 3D-partition asymptotic to $(\bx \lambda
...
\[
|\pi_{min}| = -\sum_{j=1}^{\infty} \mu_{j}^{2}.
\]
\end{proof}
...
\sfV_{\bx \lambda \emptyset}}{\sfV_{\bx \emptyset
\emptyset}\sfV_{\lambda \emptyset \emptyset}}q^{|\lambda |}
\right)^{e(B)-e(S)}\\
&\quad  \cdot \left(\sum_{\mu} p^{||\mu ||^{2}}
\frac{\sfV_{\mu '\mu \emptyset} \sfV_{\bx \mu \emptyset}}{\sfV_{\bx
\emptyset \emptyset}\sfV_{\mu \emptyset \emptyset}}q^{|\mu |}
\right)^{e(S)}.
\end{align*}
...
\left(\sum_{\lambda} (1-p)\frac{\sfV_{\bx \lambda
\emptyset}}{\sfV_{\lambda \emptyset \emptyset}} q^{|\lambda
|}\right)^{e(B)-e(S)} \\
&\quad \cdot \left(\sum_{\mu} (1-p)p^{||\mu ||^{2}} \sfV_{\mu '\mu
\emptyset}\frac{\sfV_{\bx \mu \emptyset}}{\sfV_{\mu \emptyset
\emptyset}}q^{|\mu |} \right). 
\end{align*}
Applying slight rearrangments of \cite[eqns~(2)\&(4)]{Bryan-Kool-Young} we see that 
\begin{align*}
...
It follows that the preimages of points under the map
...
\[
C = \cup_{i=1}^{n}\left(\lambda^{(i)}F_{x_{i}} \right)
\cup_{j=1}^{m}\left(\mu^{(j)}F_{y_{j}} \right) . 
\]
...
fpcq cover is simplier in this case:
\[
...
\begin{align*}
e\left(\Hilb^{\bullet}(\Xhat_{z_{j}},\hat{C}_{z_{j}}) \right)&= \sfVtilde_{\mujprime  \mu^{(j)} \emptyset },\\
...
}_{y_{j}} ) \right) & = \sfVtilde^{e(F^{\circ}_{y_{j}})}_{\mu^{(j)} \emptyset \emptyset}.
\end{align*} 
...
\frac{\sfVtilde_{\mujprime \mu^{(j)}\emptyset
}}{\sfVtilde_{\emptyset \emptyset \emptyset}} .
\end{align*}
...
\right)^{e(S)}.
\]
Sustituting in the equations in lemma~\ref{lem: normalized vertex in
...
the Behrend functionconjecture}, which says that up to an overall
...
\subsection{Proof of theorem~\ref{thm: DT(X) assuming the Behrend functionconjecture}}
...
Hilb(U,C)}) except that the later is indexed by the length of
...
assuming the Behrend functionconjecture}.

The case of $\DT_{\fiber}(X)$ (previously shown by Toda) proceeds
similarly except that it doesn't require the difficult deformation
result of next section. Indeed, in this case, we only need to know
that the value of the Behrend function at a partition thicked fiber
curve is 1:
\[
...
points on $T$ and hence smooth and even dimensional. 
...
is the key technical result require in section~\ref{sec: Behrend} to
promote our computation of $\DThat (X)$ to the Behrend function
version $\DT (X)$.
...
The value of the Berhend function at $[C]\in \Hilb (X)$ for a
...
\]
be the strata defined in theorem~\ref{thm: strata of Hilb(C^2) with
...
$(p,v)\mapsto (p,v+\theta (p))$ where $p\in B$ and $v\in
T|_{p}$. 
...
Formally, we construct a universal subscheme 
\[
...
&&\dTo_{\pi_{V}} && \dTo_{\pi_{H}} &&\\
\mathcal{B} &\Into & H^{0}\times T && V_{l}\times T& \lInto & \mathcal{Z}
\end{diagram}
\]
In the above diagram, $\mathcal{Z}\subset V_{l}\times T$ is the family
subschemes of $T$ by the universal subscheme over $\Hilb^{d}(T)$,
$\mathcal{B}\subset H^{0}\times T$ is the family of curves in $T$
given by $H^{0}$, explicitly given by the set of points $(\theta
,p,\theta (p))$. The maps $\pi_{V}$ and $\pi_{H}$ are the obvious
projections and the maps $p$ and $i$ are the projection and the zero
section of the elliptic fibration $X\to T$. We are also adopting the
general abuse of notation that we drop factors of the identity map
from the notation, that is if $f:A\to B$ we denote also by $f$ the map
$f\times Id_{C}:A\times C\to B\times C$.

Then the subscheme
\[
\mathcal{C} = i(\pi_{V}^{-1}(\mathcal{B})) \cup \left(p\circ \Theta
\circ \pi_{H} \right)^{-1}(\mathcal{Z}) \subset H^{0}\times V_{l}\times X
\]
is the desired universal subscheme over $H^{0}\times V$. \qed
...
Let $X=\Tot (K_{S})$ and let $T=\Tot (K_{S}|B)$, then we have
\[
...
\end{lemma}
\begin{proof}
By a well known fact about elliptic surfaces (see \cite[???]{Fr-Mo} or \cite[III.1.1]{Miranda}), 
\[
