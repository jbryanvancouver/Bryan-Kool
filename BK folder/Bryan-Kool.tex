...
whose Euler characteristic localizes to the $\CC^{*}$-fixed locus.
\item In section~\ref{sec: reduction to thickened comb curves} we show
that any $\CC^{*}$-invariant subscheme has a maximal Cohen-Macaulay
subscheme which is a curve of a special form which we call a partition
thickened comb curve (definition~\ref{defn: partition thickened comb
...
\Hilb^{n}\left(\bx ,\emptyset ,\lambdaiprime \right)\\
\Hilb^{n}\left(\Xhat_{y_{j}},\hat{C}_{y_{j}} \right)&\cong
\Hilb^{n}\left(\bx ,\emptyset ,\mujprime \right)\\
\Hilb^{n}\left(\Xhat_{z_{j}},\hat{C}_{z_{j}} \right)&\cong
\Hilb^{n}\left(\mu^{(j)},\emptyset ,\mujprime \right)
\end{align*}
...
\sfVtilde_{\mujprime \mu^{(j)}\emptyset  } \nonumber
\end{align}
...
\sigma_{F_{x_{i}}^{\circ }}^{-1}(np) &\cong \Hilb^{n}\left(\emptyset  ,\emptyset,\lambdaiprime  \right) \\
\sigma_{F_{y_{j}}^{\circ }}^{-1}(np) &\cong \Hilb^{n}\left(\emptyset  ,\emptyset,\mujprime  \right) 
\end{align*}
...
continaed in both the $\bx$-leg and the $\lambda$-leg. They intersect
...
$\mu '$-leg contribute $-1$. This intersection is a stack of squares
of side lengths $\mu_{1},\mu_{2},\dotsc$ and hence
\[
...
\cite[eqn~5]{Bryan-Kool-Young}, we get
\[
...
we describe how to adapt our computation of $\DThat (X)$ to obtain a
proof of the formula for $\DThat_{\fiber}(X)$ which is independent of
Toda's. 
...
\left(\sum_{\lambda}q^{|\lambda |} \right)^{e(B)-e(S)} \cdot
...
the Behrend functionconjecture}, which says that up to an overall sign,
the partition functions $\DThat (X)$ and $\DT (X)$ are equal after the
simple change of variables $y=-p$. In order to do this we will need to
assume a conjecture about the Behrend function which we formulate
below for general Calabi-Yau threefolds.
...
$\O_{Z}/\O_{C}$ whereas $\Sigmaxylambdamu$ is indexed by $\chi
(\O_{Z})$. Consequently
\[
...
The locus of comb curves is contained in the non-singular locus of $\Hilb (X)$.
\end{theorem}
...
By our assumption that $S$ is not a product, which implies
$$
