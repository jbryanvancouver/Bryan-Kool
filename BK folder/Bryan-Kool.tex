...
$$
\widehat{\DT}^{\conn}(X) = \frac{1}{(p^{\frac{1}{2}} - p^{-\frac{1}{2}})^{2}} \prod_{k=1}^{\infty} \frac{1}{(1-p q^k)^2 (1-p^{-1} q^k)^2 (1-q^k)^{12N-4}}.
$$
\end{corollary}
...
The most important result of this paper is perhaps not the formula, but rather the method of calculation. Even though the geometry under consideration is not toric, we combine $\CC^*$-localization, motivic methods, formal methods, and $\CC^{*3}$-localization to end up with expressions, which only depend on the topological vertex, $e(B)$, and $N$. Here is a rough sketch of our method:
\begin{enumerate}
...
\item[(B)] The fibres of \eqref{intromap} decompose into components according to the shape of the underlying Cohen-Macaulay curve. In Section \ref{fixedlocus}, we show this leads to a decomposition over 2D partitions $\lambda = (\lambda_0 \geq \lambda_1 \geq \cdots)$.
...
\item[(D)] On the formal neighbourhoods $\widehat{X}_x$, we have an action of $\CC^{*3}$. This allows us to express their contributions in terms of the topological vertex. The contributions of the tubular neighbourhoods along the \emph{punctured} section and fibres can also be expressed in terms of the topological vertex (roughly speaking, by utilizing actions of the elliptic curve $F$ or $\CC^*$ on itself). This is worked out in Section \ref{vertex}.
\end{enumerate}
...
\item all singular fibres of $p$ are of Kodaira type $I_1$, i.e.~rational nodal fibres. 
\end{enumerate}
...
$$
We revisit the $\CC^*$-fixed locus in detail in the Section \ref{fixedlocus}. Denote by $\Sym^d(B)$ the $d$th symmetric product of $B$. Recall that we have projections 
$$
...
$$
where $f_d := \rho_{d*}(1)$ is a constructible function on $\Sym^d(B)$. Its value at a closed point $\mathfrak{a} = \sum_i a_i x_i \in \Sym^d(B)$ is 
$$
...
In Section \ref{sym}, we considered the stratum
$$
\rho_{d}^{-1} ( \mathfrak{a} + \mathfrak{b} ) \subset \Hilb^{d, \bullet}(X)^{\CC^*},
$$
for any $\mathfrak{a} = \sum_{i=1}^{m} a_i x_i \in \Sym^{d_1}(B^{\sm})$ and $\mathfrak{b} = \sum_{j=1}^{n} b_j y_j \in \Sym^{d_2}(B^{\sing})$ satisfying $d_1+d_2=d$. Proposition \ref{ZCM} gives a decomposition of $\rho_{d}^{-1} ( \mathfrak{a} + \mathfrak{b} )$ into components\footnote{We use the term component somewhat loose: it means a subset which is both open and closed, so can be a union of actual connected components.}
...
$$
the formal neighbourhood of $F \setminus (B \times \{x\})$ inside $Y \setminus (B \times \{x\})$. Recall that $x \in F \cap B$ and $B \times \{x\} \subset \tilde{S} \subset Y$. By removing the section $B$, we have achieved an isomorphism \todo{Can provide more argument here?} 
...
Inserting these equations into \eqref{initialprod} provides the formula for $\widehat{\DT}(X)$ in Theorem \ref{main}. Following the exact same line of reasoning of Sections \ref{sym}--\ref{vertex}, it is easy to see that
\begin{align*}
...
\end{align*}
The formula for $\widehat{\DT}^{\conn}(X)$ in Theorem \ref{main} follows after dividing $\widehat{\DT}(X)$ by this expression.
...
be any function with $g (0)=1$. Let $f_{d}:\Sym ^{d} (S)\to \ZZ (\!(p)\!)$
be the constructible function defined by $f_{d} (\sum_{i}
a_{i}x_{i})=\prod _{i}g (a_{i})$. Then
\[
\sum _{d=0}^{\infty } q^{d} \int _{\Sym ^{d} (S)} f_{d} de =
\left(\sum _{a=0}^{\infty }g (a) q^{a} \right)^{e (S)}.
\]
\end{lemma}

\begin{remark} \label{MacD}
In the special case where $g=f_{d}\equiv  1$, the lemma recovers
MacDonald's formula: $$\sum _{d=0}^{\infty }e (\Sym ^{d} (S)) q^{d} =
\frac{1}{(1-q)^{e (S)}}.$$ 
...
we find
\begin{align*}
\sum _{d=0}^{\infty } q^d \int _{\Sym ^{d} (S)}f_{d}\,\,de & = \sum _{(m_{1},m_{2},\dots )} \prod _{j=1}^{\infty } \left(g (j) q^{j} \right)^{m_{j}} \binom{e (S)}{m_{1},m_{2},m_{3},\dots }\\
&=\left(1+\sum _{j=1}^{\infty }g (j) q^{j} \right)^{e (S)}
\end{align*}
