...
functionconjecture} requires theorem~\ref{thm: Ext computation}, an
...
\right)^{e(B)-e(S)}\\
&\quad \cdot \left(\sum_{\mu} p^{||\mu ||^{2}} \frac{\sfV_{\mu \mu'
...
the set of points $(\theta ,p,\theta (p))$. The maps $\pi_{V}$ and
$\pi_{H}$ are the obvious projections and the maps $p$ and $i$ are the
...
$I_{\lambda}\subset \CC [r,s]$ defined by partitions with $\lambda
\vdash d$ and in this case (because $\length 
(Z_{\lambda}\cap B)=l$) have $\lambda_{1}=l$.
...
\]
is cut out by the linear equations obtained by linearizing the
condition 
\[
\length (Z\cap \{s=0 \})=l
\]
at $Z_{\lambda}$. In
\cite{Haiman1998}, Mark Haiman has given a very explicit basis for
...
which is the furthest to the left in a row of $\lambda$.
\item  $(\rho ,\sigma )\to (\rho ',\sigma ')$ is a northwest
pointing arrow with $(\rho ,\sigma )$ located at a box just to the
left of a row of $\lambda$ and  $(\rho ',\sigma ')$ located at a box
...
consider
\[
\dim \left(\frac{\CC [r,s] }{I_{Z_{\lambda}}\cup (s)} \right).
\]
If $\sigma '>0$ then $I_{Z_{\lambda}}(\phi )\cup (s) =
I_{Z_{\lambda}}\cup (s)$: Haiman arrows $(\rho ,\sigma )\to (\rho
...
In case 2, we have 
\[
\frac{\CC [r,s]}{I_{Z_{\lambda}}(\phi )\cup (s)} \cong \frac{\CC
...
Let $X=\Tot (K_{S})$ and let $T=\Tot (K_{S}|B)$, then clearly we have
\[
...
for any fibre $F$ we have $D' \cdot F = 1$. Therefore $D'$ contains a
