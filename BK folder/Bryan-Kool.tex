...
\nu}$ differs from the usual topological vertex by an over all
normalization: 
\[
\sfVtilde_{\lambda \mu \nu} = p^{-|\pi_{min}|} \sfV_{\lambda \mu \nu}
\]
...
\]
where $||\mu ||^{2}:= \sum_{i=1}^{\infty} \mu_{i}^{2}$ .
\end{lemma}
\begin{proof}
\dots 
\end{proof}
...
the same as the formula for $\DThat (X)$ in theorem~\ref{thm: main thm
-- formulas for DT and DTfiber}. Since the formula for
...
\begin{align*}
e\left(\Hilb^{\bullet}(\Xhat_{z_{j}},\hat{C}_{z_{j}}) \right)&= \sfVtilde_{\left(\mu^{(j)} \right)' \mu^{(j)} \emptyset },\\
...
\frac{\sfVtilde_{\left(\mu^{(j)} \right)'\mu^{(j)}\emptyset
...
B\cup_{i}\left(\lambda^{(i)}F_{x_{i}} \right)$ is 
\[
(-1)^{h^{0}(N_{B/X}) }\prod_{i=1}^{n} (-1)^{\lambda_{1}^{(i)}}. 
\]
\end{corollary}
