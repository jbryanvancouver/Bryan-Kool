...
Finally, note that the generating functions $e(\Hilb^{(1,a),\bullet}(\Xhat _x)_{\lambda}^{\CC^*})$, $e(\Hilb^{(1,b),\bullet}(\Xhat _y)_{\mu}^{\CC^*})$, $e(\Hilb^{b,\bullet}(\Xhat _z)_{\mu}^{\CC^*})$ all start with $1$. On the other hand, from the definition
\begin{align*}
...
\]w
We can build many double complexes from these by applying $\Hom(I_C,\cdot)$, $\Hom(\cdot, \O_C)$ etc. \todo{Dive into those diagrams. Write up what we got so far.} \\
...
\end{axis}
\end{tikzpicture}
\caption{Cohen-Macaulay curve $Z_{\CM}$ with underlying reduced support in red and thickenings $\lambda^{(i)}$ along smooth fibers and $\mu^{(j)}$ along singular fibers.}
\end{figure}
...
\draw [ultra thick,orange] 
                   (2  ,0   ,5)
to [out=90,in=-90] (2  ,0.6 ,5)
to [out=90,in=-90] (1.5,1.5 ,5)
to [out=90,in=-90] (2  ,2.4 ,5)
to [out=90,in=-90] (2  ,3   ,5);
...
\node [above] at (3,3,3.5) {$\mu^{(i)} $};
\node [below] at (3,0,5) {$F_{y_{i}}$};
\node [right] at (3,0.6,5) {$y_{i}$};
\node [right] at (2.8,1.5,5) {$z_{i}$};
\node [above] at (1,3,4) {$\lambda^{(j)}$};
\node [below] at (1,0,5) {$F_{x_{j}}$};
\node [right] at (1,0.6,5) {$x_{j}$};
