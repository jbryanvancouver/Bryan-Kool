...
$$
Every $\CC$-valued point of $\Hilb^n(X,C)$ arises from a quotient $I_Z \twoheadrightarrow Q$ in this way. Hence we have a geometric bijection $\Quot^n_X(I_C) \rightarrow \Hilb^n(X,C)$ from which the lemma follows.
\end{proof}
...
\vdash \bolda}\, \sum_{\boldmu \vdash \boldb}  p^{\chi(\O_{C_{\boldx, \boldy, \boldlambda, \boldmu}})} \Hilb_X^{\bullet}(I_{C_{\boldx, \boldy, \boldlambda, \boldmu}}) \\
&=  \sum_{\boldlambda
...
\Quot_X^{\bullet}(I_{C_{\boldx, \boldy, \boldlambda, \boldmu}},\{z_j\}) \cdot
\Quot_X^{\bullet}(I_{C_{\boldx, \boldy, \boldlambda, \boldmu}},F^\circ_{y_j}).
\end{align*}
\end{lemma}
...
kernels of the quotients on the RHS do not need to correspond to be monomial ideals: although their underlying maximal Cohen-Macaulay
