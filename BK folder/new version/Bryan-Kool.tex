...
constructible morphism, i.e.~a constructible morphism which is bijective on $\CC$-valued points.
...
This provides a stratification of $\Quot_X^{\bullet}(I_{C},S)$ into locally closed subsets as we now show. 

\begin{lemma}\label{lem: Sigma = product of local Hilbert schemes}
The following equation holds in $K_{0}(\Var_{\CC})(\!(p)\!)$
\begin{align*}
\Quot_X^{\bullet}(I_C,S) = p^{\chi
...
\begin{proof}
TBC TBC RRRRR For any stratum $S \in \Sigma$, we define by $X_{\overline{S}}^{(n)}$ the $n$-th order neighbourhood of the closed subset $\overline{S}$ inside $X$. I.e.~if $\overline{S}$ is defined by the ideal $I_{\overline{S}} \subset \O_X$, then $X_{\overline{S}}^{(n)}$ is the closed subscheme defined by $I_{\overline{S}}^n \subset \O_X$. In particular $X_{\overline{S}}^{(1)} = \overline{S}$.

It suffices to show the equality of the proposition modulo $p^N$  and let $N \rightarrow \infty$. There exists an integer $M$ with the following property *****this may need more argument?*****: \\

\noindent for any zero dimensional closed subscheme $Z \subset X$ of length $< N$ supported on $S$, the scheme theoretic support of $Z$ is contained inside $X^{(M)}_{\overline{S}}$. \\
...
determines $\O_Z$ and there are no conditions on the overlaps. 
In other words, we have a constructible bijective morphism
