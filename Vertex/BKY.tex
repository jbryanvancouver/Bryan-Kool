...
\cite{Bryan-Kool,Bryan-K3xE,BOPY}. As we will explain below, each
...
\] 
where $e(-)$ is topological Euler charateristic and 
\[
...
\]
In the case where $X$ is toric with action $T$, then $\HilbBetan
(X)^{T}$ parameterizes $T$-invariant subschemes and is a finite
set. Thus $\DThat_{\beta ,n}(X)$ simply counts the number of $T$
invariant subschemes $Z\subset X$ with $[Z]=\beta$ and $\chi
(\O_{Z})=n$.
...
along the axes determined $(\lambda, \mu, \nu )$.
...
each coordniate chart) such that the corresponding subschemes agree on
...
induces an action on the stratum $\HilbBetan (X,C)$. This strategy can
potentially be iterated: the fixed locus $\HilbBetan
(X,C)^{\cnums^{*}}$ may be further stratified (for example by the
support of the embedded points) and these substrata may admit further
$\cnums^{*}$ actions. The upshot is that in circumstances where this
strategy is successful, it reduces the computation of the Euler
characteristic of the Hilbert scheme to the substratum of the Hilbert
scheme parameterizing subschemes which are \emph{formally locally
monomial ({\folomo }  for short)}, i.e. given by a monomial ideal in the
formal neighborhood of each point. Denoting this substratum by the
subscript \folomo, we get
\begin{align*}
...
singularity. Let $B$ be a smooth section curve, that is a non-singular
curve meeting $F$ and $N$ once each in a node. The strategy outlined
in the previous subsection works particularly well in this setting
where we consider curve classes which are multiples of the fiber class
plus possibly a section class. In these cases, the local contributions
can be expressed in terms of the topological vertex. The results are
the following
...
&= \left(p^{\half}-p^{-\half} \right)^{2g-1} p^{\half} \,
\sum_{\lambda} q^{|\lambda |} \frac{\Vsf_{\lambda \bx
\emptyset}}{\Vsf_{\lambda \emptyset \emptyset}},\\
...
&= \left(p^{\half}-p^{-\half} \right)^{2g-1} p^{\half} \, \sum_{\lambda}
...
singularitiy of $B'\cup F$ at $n$ is formally locally that of the
...
q^{|\lambda |} \frac{\Vsf_{\lambda \bx \bx}}{\Vsf_{\lambda \emptyset
\emptyset}},
\end{align*}

The appearance of the topological vertex in the \emph{denominators} of
these formulas deserves explaining. In the case of a toric Calabi-Yau
threefold, only a finite number of monomoial ideals need to be counted
--- one for each toric coordinate chart. In the case of an
elliptically fibered threefold as above, we have a monomial ideal in
the formal neighborhood of each point on our curve. The possible
locations of these monomial ideas are parameterized by various
symmetric products of our curves. Symmetric products are very amenable
to our motivic methods. Indeed, symmetric products endow the
Grothendieck group of varieties with a lambda ring structure which is
compatible with the Euler characteristic homomorphism. The vertex term
$\Vsf_{\lambda \emptyset \emptyset}$ appearing in the above formulas
counts monomial ideals at the points of $F-p$ where $p$ is the point
where the section meets $F$. Because of the symmetric product
formalism in the Grothendieck group, this term appears with an
exponent given by the topological Euler characteristic of $F-p$:
\[
(\Vsf_{\lambda \emptyset \emptyset})^{e(F-p)} =\frac{1}{\Vsf_{\lambda
\emptyset \emptyset}} .
\]

\subsection{Applying our trace formulas and the appearence of Jacobi forms.}
