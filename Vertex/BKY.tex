...
\subsection{An absurdly short recap of Donaldson-Thomas theory}
...
subscheme defined by the corresponding monomial ideal is supported on
the coordinate axes of $\cnums^{3}$ and has nilpotent thickenings
along the axes determined $(\lambda, \mu, \nu )$.

So we see that to compute $\DThat_{\beta ,n}(X)$ for a toric
Calabi-Yau threefold $X$, we need to count 3D partitions (one for each
coordniate chart) such that the corresponding subschemes agree on the
coordinate overlaps and the curve class and holomorphic Euler
...
The topological vertex can also be used to compute $\DThat (X)$ for
some Calabi-Yau threefolds $X$ which do not have a global torus
action, if it is known for example that the reduced support of all
1-dimensional subschemes is \emph{formally locally} monomial. A
typical setting where this can be applied is when $X$ is elliptically
fibered. Namely, there is a map $\pi :X\to  S$ to a surface such that
the fibers are possibly nodal genus 1 curves. We typically require a
section $\sigma :S\to X$ and we consider curve classes of the form
\[
\beta_{d} = \sigma_{*}(\beta ) + d [F]
\]
or simply
\[
\tilde{\beta}_{d} = d[F].
\]
Here $\beta$ is an irreducible curve class in $S$ and $[F]$ is the
class of the fiber.

The basic idea is to exploit the \emph{motivic} nature of Euler
characteristic. The Euler characteristic defines a ring homomorphism
from the Grothendieck group of varieties to the integers. More
prosaically, this means that one can compute the Euler characteristic
of a scheme by stratifying the scheme and then summing the Euler
characteristics of each individual stratum. If we stratify $\HilbBetan
(X)$ by the support of the corresponding subscheme, it then makes
sense to talk about the contribution of schemes supported on some
particular curve to the Donaldson-Thomas invariant of $X$. For
example, given a particular fiber $F$ and some particular curve
$C\subset S$ in the class, it makes sense to talk about the
contribution of schemes supported on $\sigma (C)\cup F$ to the
invariant $\DThat_{\beta_{d},n}(X)$. Namely, it is the Euler
characteristic of the substratum of the Hilbert scheme $\HilbBetan
(X)$ which parameterizes subschemes supported on  $\sigma (C)\cup F$.
